            ...
            {fontList.length === 0 ? (
              ...
              <div className="space-y-3">
                 ...
                 <div className="space-y-2">
                    {fontList.map((f, idx) => (
                        <div key={idx} onClick={() => switchFont(idx)} className={cn("group flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-all", activeFontIndex === idx ? "bg-violet-50 border-violet-200 dark:bg-violet-900/30 dark:border-violet-700" : "bg-white border-gray-100 dark:bg-gray-800 dark:border-gray-700 hover:border-violet-200")}>
                            <div className={cn("p-1.5 rounded", activeFontIndex === idx ? "bg-violet-200 dark:bg-violet-800 text-violet-700 dark:text-violet-200" : "bg-gray-100 dark:bg-gray-700 text-gray-500")}><FileType size={14} /></div>
                            <div className="flex-1 min-w-0"><div className={cn("text-xs font-semibold truncate", activeFontIndex === idx ? "text-violet-900 dark:text-violet-100" : "text-gray-700 dark:text-gray-300")}>{f.name}</div><div className="text-[10px] text-gray-400">{f.glyphs.length} Glyphs</div></div>
                            <button onClick={(e) => removeFont(e, idx)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-400 rounded transition-all" title="Remove font"><X size={12} /></button>
                        </div>
                    ))}
                 </div>
                 {fontList.length > 1 && (
                    <div className="pt-2 border-t border-dashed border-gray-200 dark:border-gray-700">
                        <label className="text-xs font-bold text-gray-400 uppercase flex items-center gap-2 mb-2"><SplitSquareHorizontal size={12}/> Compare With</label>
                        <select value={compareFontIndex === null ? -1 : compareFontIndex} onChange={(e) => setCompareFontIndex(e.target.value === '-1' ? null : Number(e.target.value))} className="w-full text-xs p-2 rounded bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 outline-none">
                            <option value="-1">-- None --</option>
                            {fontList.map((f, idx) => (idx !== activeFontIndex && (<option key={idx} value={idx}>{f.name}</option>)))}
                        </select>
                        {compareFontIndex !== null && (<div className="mt-2 space-y-2 p-2 bg-gray-50 dark:bg-gray-800 rounded border border-gray-100 dark:border-gray-700 animate-in slide-in-from-top-1"><div className="flex items-center gap-2"><span className="text-[10px] text-gray-500 w-16">Overlay Color</span><input type="color" value={currentSettings.comparisonColor} onChange={(e) => updateSetting('comparisonColor', e.target.value)} className="h-5 w-5 border-none rounded cursor-pointer bg-transparent" /></div><SliderControl label="Overlay Opacity" value={currentSettings.comparisonOpacity} min={0.1} max={1} step={0.1} onChange={(v) => updateSetting('comparisonOpacity', v)} onAfterChange={commitSettingChange}/></div>)}
                    </div>
                 )}
              </div>
            )}
        ...
        <div className="p-4 space-y-6">
             ...
             <div className="grid grid-cols-2 gap-2">
               <div className="col-span-2 grid grid-cols-3 gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                 {['svg', 'png', 'both'].map(fmt => (
                   <button key={fmt} onClick={() => setExportFormat(fmt)} className={cn("px-2 py-1.5 text-xs font-medium rounded-md capitalize transition-all", exportFormat === fmt ? "bg-white dark:bg-gray-700 text-violet-600 shadow-sm" : "text-gray-500 hover:text-gray-800 dark:hover:text-gray-300")}>{fmt}</button>
                 ))}
               </div>
               {(exportFormat === 'png' || exportFormat === 'both') && (
                 <div className="col-span-2 flex items-center justify-between bg-gray-50 dark:bg-gray-800 p-2 rounded-lg border border-gray-100 dark:border-gray-700"><span className="text-xs text-gray-500">PNG Size:</span><div className="flex gap-1">{[1, 2, 4].map(scale => (<button key={scale} onClick={() => setPngScale(scale)} className={cn("px-2 py-0.5 text-[10px] font-bold rounded border transition-colors", pngScale === scale ? "bg-violet-100 text-violet-700 border-violet-200" : "bg-white text-gray-500 border-gray-200")}>{scale}x</button>))}</div></div>
               )}
             </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Move X</label><input type="number" value={currentSettings.translateX} onChange={(e) => updateSetting('translateX', Number(e.target.value))} className="w-full p-2 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500"/></div>
              <div className="space-y-1"><label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Move Y</label><input type="number" value={currentSettings.translateY} onChange={(e) => updateSetting('translateY', Number(e.target.value))} className="w-full p-2 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500"/></div>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-gray-800">
              ...
              <div className="flex items-center gap-2"><div className="w-20 text-[10px] text-gray-500 font-semibold">Fill Color</div><div className="flex-1 flex items-center gap-2 p-1 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800"><input type="color" value={currentSettings.color} onChange={(e) => updateSetting('color', e.target.value)} onBlur={commitSettingChange} className="h-5 w-6 border-none rounded cursor-pointer bg-transparent" /><input type="text" value={currentSettings.color} onChange={(e) => updateSetting('color', e.target.value)} onBlur={commitSettingChange} className="flex-1 bg-transparent text-xs font-mono focus:outline-none uppercase" /></div></div>
              <div className="flex items-center gap-2"><div className="w-20 text-[10px] text-gray-500 font-semibold">Background</div><div className="flex-1 flex items-center gap-1"><button onClick={() => { updateSetting('backgroundColor', 'transparent'); commitSettingChange(); }} className={cn("h-6 w-6 rounded border flex items-center justify-center text-[10px]", currentSettings.backgroundColor === 'transparent' ? "border-violet-500 ring-1 ring-violet-500" : "border-gray-200")} title="Transparent"><div className="w-full h-full bg-[length:4px_4px]" style={{ backgroundImage: 'radial-gradient(#ccc 1px, transparent 1px)' }}></div></button><button onClick={() => { updateSetting('backgroundColor', '#FFFFFF'); commitSettingChange(); }} className={cn("h-6 w-6 rounded border bg-white", currentSettings.backgroundColor === '#FFFFFF' ? "border-violet-500 ring-1 ring-violet-500" : "border-gray-200")} title="White"></button><button onClick={() => { updateSetting('backgroundColor', '#000000'); commitSettingChange(); }} className={cn("h-6 w-6 rounded border bg-black", currentSettings.backgroundColor === '#000000' ? "border-violet-500 ring-1 ring-violet-500" : "border-gray-600")} title="Black"></button><div className={cn("w-28 flex items-center gap-1 p-1 border rounded-lg bg-white dark:bg-gray-800", currentSettings.backgroundColor !== 'transparent' && currentSettings.backgroundColor !== '#FFFFFF' && currentSettings.backgroundColor !== '#000000' ? "border-violet-500" : "border-gray-200 dark:border-gray-700")}><input type="color" value={currentSettings.backgroundColor === 'transparent' ? '#ffffff' : currentSettings.backgroundColor} onChange={(e) => updateSetting('backgroundColor', e.target.value)} onBlur={commitSettingChange} className="h-6 w-6 border-none rounded cursor-pointer bg-transparent shrink-0" /><input type="text" value={currentSettings.backgroundColor === 'transparent' ? '' : currentSettings.backgroundColor} placeholder="HEX" onChange={(e) => updateSetting('backgroundColor', e.target.value)} onBlur={commitSettingChange} className="flex-1 min-w-0 bg-transparent text-[10px] font-mono focus:outline-none uppercase" /></div></div></div>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-gray-100 dark:border-gray-800">
               ...
               <div className="space-y-1"><label className="text-[9px] text-gray-500">For Unicode Glyphs</label><div className="flex items-center gap-2 p-1 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800"><input type="text" value={filenamePattern} onChange={(e) => setFilenamePattern(e.target.value)} className="flex-1 p-1 bg-transparent text-xs font-mono focus:outline-none" placeholder="U+{hex}" /></div></div>
          ...
          {font && (
             <div className="space-y-3 pt-4 border-t border-gray-100 dark:border-gray-800">
               ...
               <label className="text-xs font-bold text-gray-400 uppercase flex items-center justify-between"><span>Detected Blocks</span><span className="bg-gray-100 dark:bg-gray-800 text-gray-600 px-1.5 rounded text-[10px]">{blocks.length}</span></label>
                   ...
                   {blocks.map((block, idx) => {
                     ...
                     <div key={idx} className={cn("group flex flex-col p-2 rounded-lg border transition-all", activeBlock === block ? "bg-violet-50 border-violet-200 dark:bg-violet-900/20 dark:border-violet-800" : "bg-white border-transparent hover:border-gray-200 dark:bg-gray-800/50 dark:hover:border-gray-700")}>
                        <div className="flex justify-between items-start mb-1">
                           <div><div className={cn("text-xs font-semibold truncate max-w-[150px]", block.glyphs.some(g => errorIndices.has(g.index)) ? "text-red-600 dark:text-red-400" : "text-gray-700 dark:text-gray-200")} title={block.name}>{block.name}</div><div className="text-[10px] font-mono text-gray-400">{formatRange(block.originalStart || block.start, block.originalEnd || block.end)}</div></div>
                        ...
                        <div className="flex items-center gap-1 mt-1 opacity-40 group-hover:opacity-100 transition-opacity">
                           ...
                           <button onClick={(e) => { e.stopPropagation(); const newSet = new Set(selectedIndices); if (isAllSelected) block.glyphs.forEach(g => newSet.delete(g.index)); else block.glyphs.forEach(g => newSet.add(g.index)); setSelectedIndices(newSet); }} title={isAllSelected ? "Deselect Block" : "Select Block"} className={cn("p-1 rounded transition-colors", isAllSelected ? "text-violet-600 hover:bg-violet-100 dark:hover:bg-violet-900" : "text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700")}>{isAllSelected ? <CheckSquare size={14} /> : <Square size={14} />}</button>
                           <div className="ml-auto flex items-center gap-1"><button onClick={() => deleteBlock(block)} title="Delete Block" className="p-1 hover:bg-red-100 dark:hover:bg-red-900 rounded text-red-500"><Trash2 size={14}/></button><button onClick={() => handleDownload('block', block)} title="Download Block" className="p-1 hover:bg-green-100 dark:hover:bg-green-900 rounded text-green-600"><Download size={14}/></button></div>
                        </div>
                     </div>
                   )})}
               </div>
