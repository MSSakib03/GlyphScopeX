...
import { ChevronDown, ChevronUp, AlertTriangle, Trash2, CheckSquare, Square, Download, Eye } from 'lucide-react';
import { cn, formatRange } from '../utils/utils';
import { Button } from './UIComponents';
import GlyphCard from './GlyphCard';

const BlockSection = ({ block, collapsedBlocks, setCollapsedBlocks, errorIndices, selectedIndices, setSelectedIndices, deleteBlock, handleDownload, setViewMode, setActiveBlock, getSettingsForGlyph, toggleSelection, handleUpdateGlyphPos, handleDragEnd }) => {
    ...
    <div className={cn("mb-8 border rounded-xl overflow-hidden shadow-sm transition-colors", hasError ? "border-red-200 dark:border-red-900 bg-red-50/50 dark:bg-red-900/10" : "border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50")}>
      <div 
        ...
        onClick={() => {
             ...
             if (newSet.has(collapseKey)) newSet.delete(collapseKey); else newSet.add(collapseKey);
      ...
      >
        <div className="flex items-center gap-3">
            ...
            <div>
               <h3 className={cn("font-bold flex items-center gap-2", hasError ? "text-red-600 dark:text-red-400" : "text-gray-800 dark:text-gray-200")}>
                   {block.name}
                   {hasError && <AlertTriangle size={14} className="animate-pulse"/>}
               </h3>
               <span className="text-xs text-gray-500 font-mono">
                   {formatRange(block.originalStart || block.start, block.originalEnd || block.end)} • {block.count} Glyphs
               </span>
            </div>
        </div>
        <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
            {!block.isPartial && (
               <Button variant="ghost" size="icon" onClick={() => deleteBlock(block)} className="text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30" title="Delete Block">
                  <Trash2 size={16} />
               </Button>
            )}
            ...
            <Button variant="ghost" size="icon" onClick={() => { 
               ...
               setSelectedIndices(newSet);
                ...
                {isAllSelected ? <CheckSquare size={18} className="text-violet-600"/> : <Square size={18} className="text-gray-400"/>}
            </Button>
