...
// --- NEW COMPONENT: EXPORT OPTIONS MODAL ---
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 flex justify-end gap-3">
                    ...
                    <Button variant="primary" onClick={onConfirm}>Export</Button>
                </div>
            </div>
        </div>
    );
...
export default function GlyphForge() {
  ...
  useEffect(() => { const h = (e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'z') { e.preventDefault(); handleUndo(); } if ((e.ctrlKey || e.metaKey) && e.key === 'y') { e.preventDefault(); handleRedo(); } }; window.addEventListener('keydown', h); return () => window.removeEventListener('keydown', h); }, [handleUndo, handleRedo]);
  ...
  const filteredGlyphs = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(g => {
        ...
        return matchesName || matchesUniName || matchesHex;
      });
    }
    return result;
  }, [glyphs, searchTerm]);
  ...
  const checkClip = useCallback((g, explicitOverrides = null) => {
      ...
      if (s.positioning === 'center') {
         yPos = (s.canvasHeight / 2) + s.translateY + ((raw.y2 - raw.y1) * fontScale / 2) - (raw.y2 * fontScale);
      ...
      const finalX1 = xPos + (raw.x1 * fontScale); const finalY1 = yPos - (raw.y2 * fontScale);
      const finalX2 = xPos + (raw.x2 * fontScale); const finalY2 = yPos - (raw.y1 * fontScale);
      const ry1 = Math.min(finalY1, finalY2); const ry2 = Math.max(finalY1, finalY2);
      const T = 0.1;
      const mathClip = finalX1 < -T || ry1 < -T || finalX2 > (s.canvasWidth + T) || ry2 > (s.canvasHeight + T);
      if (!mathClip) return false;
      const mp = g.glyph.getPath(xPos, yPos, scaledFontSize);
      const bb = mp.getBoundingBox();
      return bb.x1 < -T || bb.y1 < -T || bb.x2 > (s.canvasWidth + T) || bb.y2 > (s.canvasHeight + T);
        ...
        reader.onload = (event) => {
            try {
                ...
                const processGlyphBatch = () => {
                    ...
                    gIndex = end;
                    if (gIndex < totalGlyphs) {
                        setTimeout(processGlyphBatch, 0); 
                    } else {
                        newFonts.push({ name: file.name.replace(/\.(ttf|otf|woff)$/i, ''), fileName: file.name, fontObj: parsedFont, glyphs: glyphList, originalCount: glyphList.length });
                        fileIndex++;
                        setTimeout(processNextFile, 0); 
                    }
                };
  ...
  const handleSidebarBlockClick = (block) => {
    if (!block) return;
  ...
  const handleSubsetClick = () => {
      ...
      if (!hasSelection && !isModified) {
          alert("Please select or remove glyphs or unicode blocks to subset first.");
          return;
      }
  ...
  const executeSubsetExport = () => {
      setSubsetModal({ ...subsetModal, isOpen: false });
      setIsProcessing(true);
      
      const currentFontData = fontList[activeFontIndex];
      const targets = selectedIndices.size > 0 
          ? currentFontData.glyphs.filter(g => selectedIndices.has(g.index)) 
          : currentFontData.glyphs; 

      const targetIndices = new Set(targets.map(g => g.index));

      setTimeout(() => {
          try { 
              const newFont = createSubset(font, targetIndices); 
...
// --- SPRITE FEATURE LOGIC ---
  const handleSpriteClick = () => {
      setSpriteModal({ ...spriteModal, isOpen: true });
  };

  const executeSpriteExport = async () => {
      setSpriteModal({ ...spriteModal, isOpen: false });
      setIsProcessing(true);
      
      const targets = selectedIndices.size > 0 
          ? glyphs.filter(g => selectedIndices.has(g.index))
          : glyphs; 
      ...
      try {
          ...
          // --- SMART RATIO LOGIC START ---
          if (COUNT < 1000) {
              // Tier 1: < 1000 Glyphs -> 3:5 Ratio (0.6)
              // Taller portrait mode for smaller sets
              COLS = Math.ceil(Math.sqrt(COUNT * 0.6));
          } else if (COUNT <= 1500) {
              // Tier 2: 1000-1500 Glyphs -> 3:4 Ratio (0.75)
              // Standard portrait mode
              COLS = Math.ceil(Math.sqrt(COUNT * 0.75));
          } else {
              // Tier 3: > 1500 Glyphs -> 1:1 Ratio (1.0)
              // Square mode for maximum packing efficiency
              COLS = Math.ceil(Math.sqrt(COUNT));
          }
          
          // Safety: Ensure at least 1 column
          COLS = Math.max(1, COLS);
          // --- SMART RATIO LOGIC END ---

          let ROWS = Math.ceil(COUNT / COLS);
          
          let IMG_WIDTH = COLS * CELL_SIZE;
          let IMG_HEIGHT = ROWS * CELL_SIZE;

          // Auto-Scale Down if dimensions exceed browser limits
          if (IMG_WIDTH > MAX_DIM || IMG_HEIGHT > MAX_DIM) {
              const maxSide = Math.max(IMG_WIDTH, IMG_HEIGHT);
              const scaleRatio = MAX_DIM / maxSide;
              
              CELL_SIZE = Math.floor(CELL_SIZE * scaleRatio);
              FONT_SIZE = Math.floor(FONT_SIZE * scaleRatio);
              ...
              // Extreme case fallback
              if (IMG_HEIGHT > MAX_DIM) {
                  COLS = Math.ceil((COUNT * CELL_SIZE) / MAX_DIM) + 5; 
          ...
          const baseName = font.names.fontFamily?.en || 'Sprite';
          const metaData = {};
          ...
          if (spriteModal.formats.png) {
              ...
              targets.forEach((g, i) => {
                  ...
                  const path = g.glyph.getPath(0, 0, FONT_SIZE);
                  const bbox = path.getBoundingBox();
                  const w = bbox.x2 - bbox.x1;
                  const h = bbox.y2 - bbox.y1;
                  
                  const cx = x + (CELL_SIZE - w) / 2 - bbox.x1;
                  const cy = y + (CELL_SIZE - h) / 2 - bbox.y1; 

                  const renderPath = g.glyph.getPath(cx, cy, FONT_SIZE);
                  renderPath.fill = '#000000';
                  renderPath.draw(ctx);

                  metaData[g.unicode || g.name] = { x, y, w: CELL_SIZE, h: CELL_SIZE, index: i };
              });

              canvas.toBlob(blob => {
                  if(blob) saveAs(blob, `${baseName}_sprite.png`);
                  else alert("Image too large. Try selecting fewer glyphs.");
              }, 'image/png');
          }

          // 2. GENERATE JSON
          if (spriteModal.formats.json) {
              if(Object.keys(metaData).length === 0) {
                  targets.forEach((g, i) => {
                      const col = i % COLS;
                      const row = Math.floor(i / COLS);
                      metaData[g.unicode || g.name] = { x: col * CELL_SIZE, y: row * CELL_SIZE, w: CELL_SIZE, h: CELL_SIZE, index: i };
                  });
              }
          ...
          // 3. GENERATE SVG
          if (spriteModal.formats.svg) {
              let svgContent = `<svg width="${IMG_WIDTH}" height="${IMG_HEIGHT}" viewBox="0 0 ${IMG_WIDTH} ${IMG_HEIGHT}" xmlns="http://www.w3.org/2000/svg">`;
              ...
              targets.forEach((g, i) => {
                  ...
                  const tempPath = g.glyph.getPath(0, 0, FONT_SIZE);
                  ...
                  const pathData = g.glyph.getPath(cx, cy, FONT_SIZE).toPathData(2);
                  svgContent += `<path d="${pathData}" fill="black" />`;
              });
              
              svgContent += `</svg>`;
              saveAs(new Blob([svgContent], {type: "image/svg+xml;charset=utf-8"}), `${baseName}_sprite.svg`);
          }

      } catch (e) { 
          console.error(e); 
          alert("Error: Font too large or browser memory limit reached."); 
      }
      setIsProcessing(false);
  };

  const executeDeleteGlyphs = (indices) => {
    scanCancelRef.current = true;
    const curF = fontList[activeFontIndex];
    const nG = curF.glyphs.filter(g => !indices.has(g.index));
    const nL = [...fontList]; nL[activeFontIndex] = { ...curF, glyphs: nG };
    setFontList(nL); setSelectedIndices(new Set());
    setErrorIndices(p => { const n = new Set(p); indices.forEach(i => n.delete(i)); return n; });
    if(activeBlock) { const ex = nG.some(g => activeBlock.glyphs.includes(g)); if(!ex) setViewMode('all'); }
  };
  ...
  const updateSetting = (key, val) => {
    if (selectedIndices.size > 0) {
      const nO = { ...overrides }; const sG = [];
      selectedIndices.forEach(i => { nO[i] = { ...(nO[i] || globalSettings), [key]: val }; const g = glyphs.find(x => x.index === i); if(g) sG.push(g); });
      setOverrides(nO); performScan(sG, nO);
    } else { setGlobalSettings(p => ({ ...p, [key]: val })); }
  };
  ...
  const performAutoFit = () => {
    let t = []; if(selectedIndices.size > 0) t = glyphs.filter(g => selectedIndices.has(g.index)); else t = getVisibleGlyphsSafe();
    if(t.length === 0) return;
    const nO = { ...overrides };
    t.forEach(g => {
        if(!g.glyph.path || g.glyph.path.commands.length === 0) return;
        const b = g.glyph.getBoundingBox(); const w = b.x2 - b.x1; const h = b.y2 - b.y1; if(w===0||h===0) return;
        const scale = (g.fontMetrics.unitsPerEm / Math.max(w,h));
        const s = nO[g.index] || globalSettings;
        nO[g.index] = { ...s, scale, translateX: 0, translateY: 0, positioning: 'center' };
    });
    setOverrides(nO); performScan(t, nO); saveToHistory(globalSettings, nO);
  };
  ...
  const generateSVGString = (glyph, settings) => {
    ...
    let yPos;
    if (settings.positioning === 'center') { yPos = (settings.canvasHeight / 2) + settings.translateY + (bbox.y2 - bbox.y1) / 2 - bbox.y2; } 
    else if (settings.positioning === 'metrics' && glyph.fontMetrics) { 
        const totalH = glyph.fontMetrics.ascender - glyph.fontMetrics.descender; 
        const blY = (settings.canvasHeight - (settings.canvasHeight * padFactor)) / 2 + ((settings.canvasHeight * padFactor) * (glyph.fontMetrics.ascender / totalH)); 
        yPos = blY + settings.translateY; 
    } else { yPos = (settings.canvasHeight * 0.65) + settings.translateY; }
    const d = glyph.glyph.getPath(xPos, yPos, scaledFontSize).toPathData(2);
    const cx = settings.canvasWidth / 2; const cy = settings.canvasHeight / 2;
    ...
    if (settings.renderMode === 'outline') { fill = 'none'; strokeColor = settings.color; strokeW = settings.strokeWidth <= 1 ? 1 : finalStrokeWidth; } 
    else if (settings.renderMode === 'negative') { fill = settings.backgroundColor === 'transparent' ? '#ffffff' : settings.backgroundColor; strokeColor = fill; strokeW = finalStrokeWidth; bgRect = `<rect width="100%" height="100%" fill="${settings.color}"/>`; } 
    else { fill = settings.color; strokeColor = settings.strokeWidth < 1 ? settings.backgroundColor : settings.color; strokeW = finalStrokeWidth; }
  ...
  const handleDownload = async (scope, targetBlock = null) => {
    ...
    try {
      ...
      else if (scope === 'view') targets = getVisibleGlyphsSafe();
      ...
      if (scope === 'selection') modeLabel = 'selected';
      ...
      for (const g of targets) {
        ...
        if (!g.unicode && filterMode === 'all') patternToUse = ligaturePattern;
        let fileNameBase = patternToUse.replace('{fontName}', fontName).replace('{name}', g.unicodeName || g.name || `glyph_${g.index}`).replace('{hex}', g.unicode ? g.unicode.toString(16).toUpperCase().padStart(4, '0') : 'NONE').replace('{index}', g.index);
        ...
        if (exportFormat === 'png' || exportFormat === 'both') {
          try { const pngBlob = await svgToPng(svgString, settings.canvasWidth, settings.canvasHeight, pngScale); folder.file(`${fileNameBase}.png`, pngBlob); } catch (e) { console.error(`Failed to convert`, e); }
        }
      }
  ...
  const getVisibleGlyphsSafe = useCallback(() => {
    ...
    if (viewMode === 'block') {
       if (!activeBlock || !activeBlock.glyphs) return [];
  ...
  return (
    <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={cn("h-screen flex bg-gray-50 dark:bg-gray-950 text-gray-800 dark:text-gray-100 font-sans relative", darkMode ? "dark" : "")}>
      ...
      {/* SUBSET EXPORT MODAL */}
      <ExportOptionsModal 
        isOpen={subsetModal.isOpen} 
        title="Export Subset Font" 
        onClose={() => setSubsetModal(p => ({...p, isOpen: false}))}
        onConfirm={executeSubsetExport}
      >
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
            Generating subset with <span className="font-bold">{selectedIndices.size > 0 ? selectedIndices.size : fontList[activeFontIndex]?.glyphs?.length}</span> glyphs.
        </p>
        ...
        <div className="space-y-2">
            <label className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
                <input type="checkbox" checked={subsetModal.formats.ttf} onChange={(e) => setSubsetModal(p => ({...p, formats: {...p.formats, ttf: e.target.checked}}))} className="w-4 h-4 text-violet-600 rounded"/>
                <span className="flex-1 font-medium">TrueType (.ttf)</span>
            </label>
            <label className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
                <input type="checkbox" checked={subsetModal.formats.otf} onChange={(e) => setSubsetModal(p => ({...p, formats: {...p.formats, otf: e.target.checked}}))} className="w-4 h-4 text-violet-600 rounded"/>
                <span className="flex-1 font-medium">OpenType (.otf)</span>
            </label>
            <label className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
                <input type="checkbox" checked={subsetModal.formats.woff} onChange={(e) => setSubsetModal(p => ({...p, formats: {...p.formats, woff: e.target.checked}}))} className="w-4 h-4 text-violet-600 rounded"/>
                <span className="flex-1 font-medium">Web Font (.woff)</span>
            </label>
        </div>
      </ExportOptionsModal>
      ...
      <ExportOptionsModal 
        ...
        onClose={() => setSpriteModal(p => ({...p, isOpen: false}))}
        onConfirm={executeSpriteExport}
      >
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
            Generating sprite from <span className="font-bold">{selectedIndices.size > 0 ? selectedIndices.size : "ALL"}</span> glyphs.
        </p>
        <label className="text-xs font-bold text-gray-500 uppercase mb-2 block">Output Files</label>
        <div className="grid grid-cols-2 gap-2 mb-4">
            <label className="flex items-center gap-2 p-2 rounded border border-gray-200 dark:border-gray-700 cursor-pointer">
                <input type="checkbox" checked={spriteModal.formats.png} onChange={(e) => setSpriteModal(p => ({...p, formats: {...p.formats, png: e.target.checked}}))} className="w-4 h-4 text-violet-600 rounded"/>
                <FileImage size={16}/> PNG
            </label>
            <label className="flex items-center gap-2 p-2 rounded border border-gray-200 dark:border-gray-700 cursor-pointer">
                <input type="checkbox" checked={spriteModal.formats.svg} onChange={(e) => setSpriteModal(p => ({...p, formats: {...p.formats, svg: e.target.checked}}))} className="w-4 h-4 text-violet-600 rounded"/>
                <FileType size={16}/> SVG
            </label>
            <label className="flex items-center gap-2 p-2 rounded border border-gray-200 dark:border-gray-700 cursor-pointer">
                <input type="checkbox" checked={spriteModal.formats.json} onChange={(e) => setSpriteModal(p => ({...p, formats: {...p.formats, json: e.target.checked}}))} className="w-4 h-4 text-violet-600 rounded"/>
                <FileJson size={16}/> JSON
            </label>
        </div>
        {spriteModal.formats.png && (
            <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={spriteModal.transparent} onChange={(e) => setSpriteModal(p => ({...p, transparent: e.target.checked}))} className="w-4 h-4 text-violet-600 rounded"/>
                    <span className="text-sm">Transparent Background (PNG)</span>
                </label>
            </div>
        )}
      </ExportOptionsModal>
      ...
      <Sidebar 
        ...
        setActiveBlock={handleSidebarBlockClick} 
        paginatedBlocks={paginatedBlocks} currentPage={currentPage} setCurrentPage={setCurrentPage} setViewMode={setViewMode} 
        ...
        handleTypeTester={handleTypeTester} 
        handleSubsetExport={handleSubsetClick} 
        handleSpriteExport={handleSpriteClick} 
      />
        ...
        <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-6 shrink-0 z-10 gap-4">
          <div className="flex items-center gap-4 flex-1">
             ...
             <div className="relative max-w-md w-full">
                ...
                <input type="text" placeholder="Search glyphs..." value={searchTerm} onChange={handleSearchChange} className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-gray-800 border-none rounded-lg text-sm focus:ring-2 focus:ring-violet-500 outline-none" />
             </div>
               ...
               {(viewMode === 'block' || viewMode === 'errors') && (
                 <div>
                     ...
                     <div className="grid gap-4 grid-cols-[repeat(auto-fill,minmax(120px,1fr))] pb-32">
                        {getVisibleGlyphsSafe().map(g => (<GlyphCard key={g.index} g={g} comparisonG={comparisonMap ? comparisonMap.get(g.unicode || g.name) : null} settings={getSettingsForGlyph(g.index)} isSelected={selectedIndices.has(g.index)} isError={errorIndices.has(g.index)} toggleSelection={toggleSelection} onUpdatePosition={handleUpdateGlyphPos} onDragEnd={handleDragEnd} showGuidelines={showGuidelines} />))}
                     </div>
                 </div>
               )}
               {viewMode === 'all' && (
                 <div className="pb-32 min-h-full">
                     {paginatedBlocks[currentPage - 1]?.map((block, idx) => (<BlockSection key={`${currentPage}-${idx}`} block={block} collapsedBlocks={collapsedBlocks} setCollapsedBlocks={setCollapsedBlocks} errorIndices={errorIndices} selectedIndices={selectedIndices} setSelectedIndices={setSelectedIndices} deleteBlock={requestDeleteBlock} handleDownload={handleDownload} setViewMode={setViewMode} setActiveBlock={handleSidebarBlockClick} getSettingsForGlyph={getSettingsForGlyph} toggleSelection={toggleSelection} handleUpdateGlyphPos={handleUpdateGlyphPos} handleDragEnd={handleDragEnd} setCurrentPage={setCurrentPage} comparisonMap={comparisonMap} showGuidelines={showGuidelines} />))}
